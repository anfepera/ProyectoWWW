...
(function(c){
    function k(a,b){
        ...
        var b=typeof a=="string",d=Array.prototype.slice.call(arguments,1);
    ...
    c.jPlayer.convertTime=function(a){
        var b=new Date(a),d=b.getUTCHours();
        a=b.getUTCMinutes();
        b=b.getUTCSeconds();
        ...
        return(c.jPlayer.timeFormat.showHour?d+c.jPlayer.timeFormat.sepHour:
    ...
    c.jPlayer.prototype={
        _init:function(){
            ...
            c.jPlayer.count++;
            if(this.config.ready!=undefined)if(c.isFunction(this.config.ready))this.jPlayerReadyCustom=this.config.ready;else this._warning("Constructor's ready option is not a function.");
            ...
            c.extend(this.config,{
                html5:!!(this.config.oggSupport?this.config.canPlayOGG?
            ...
            var d={
                setButtons:function(g,f){
                    ...
                    if(a.config.cssId.play!=undefined&&a.config.cssId.pause!=undefined)if(f){
                        ...
                        a.config.cssSelector.play.css("display",
                            a.config.cssDisplay.play);
...
playHead:function(g,f){
    try{
        a._getMovie().fl_play_head_mp3(f)&&b.trigger("jPlayer.setButtons",
            true)
        }catch(j){
        a._flashError(j)
        }
...
playHead:function(g,f){
    if(a.config.isFileSet)try{
        ...
        else if(a.config.audio.duration>0&&!isNaN(a.config.audio.duration))a.config.audio.currentTime=f*a.config.audio.duration/100;else throw"e";
...
this.config.usingFlash?c.extend(d,
    e):c.extend(d,h);
for(var i in d){
    e="jPlayer."+i;
...
cssId:function(a,b){
    ...
    if(typeof b=="string")if(c.jPlayer._cssId[a]){
        this.config.cssId[a]!=undefined&&
        this.config.cssSelector[a].unbind("click",this.config.clickHandler[a]);
...
onProgressChange:function(a){
    if(c.isFunction(a))this.onProgressChangeCustom=a;else this._warning("onProgressChange parameter is not a function.")
        },
...
a?this.jPlayerOnProgressChange(h,
    0,0,0,d):this.jPlayerOnProgressChange(h,i,e,b,d)
},
volumeMin:function(){
    this.volume(0)
    },
...
onSoundComplete:function(a){
    if(c.isFunction(a))this.onSoundCompleteCustom=a;else this._warning("onSoundComplete parameter is not a function.")
        },
