     ...
     $('#mp_content_wrapper').find('.mp_addpl').bind('click',function(){
        ...
        if(!$esLista)
        {
            ...
            for (var i in nombreListas)
            {
                ...
                elegirListas +="<option>"+nombreListas[i]+"</option>"
            }
...
function cargar(nombreLista)
{
    ...
    { 
        ...
        $cadena2 +=" <div id=\""+listasReproduccion[j][0]+"\" class=\"mp_songs\"></div></div>";  
    ...
    for(var i in listasReproduccion)
    {
        for(var j in canciones)
        {
            ...
            $cadena +="<span class=\"mp_id\">"+canciones[j][3]+"</span>";
            $cadena +="<span class=\"mp_price\">"+canciones[j][2]+"</span></div></div>";
        }
        document.getElementById(listasReproduccion[i][0]).innerHTML= $cadena; 
